...
class Database(object):
    ...
    def new(self, name : str, contact : str, hashed_password : bytes):
        ...
        sql = f"INSERT INTO all_users values('{name}', '{contact}', '{hashed_password.decode('utf-8')}', '../static/img/avatars/{rnd_avatar}.jpg', 'Привет я пользуюсь сайтом Allelleo!', 0, 0, 0, 0)"
        self.__cursor__.execute(sql)
        self.__connection__.commit()
    ...
    def updateMainPhotoPath(self, name, path, close=False):
        sql = f"UPDATE all_users SET avatar='{path}' WHERE name='{name}'"
        self.__cursor__.execute(sql)
        self.__connection__.commit()
        if close: self.close()
